...
while(1)
{
	...
	while(key_value	== 1)key_value = GPIOPinRead(GPIO_PORTJ_BASE,GPIO_PIN_0);
	while(1)
		{
	...
	GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_0, 0x0);							// Turn off the LED.
	while(key_value	== 0)key_value = GPIOPinRead(GPIO_PORTJ_BASE,GPIO_PIN_0);	
	while(key_value	== 1)key_value = GPIOPinRead(GPIO_PORTJ_BASE,GPIO_PIN_0);
	while(1)
		{
...
void S800_GPIO_Init(void)
{
  ...
  GPIOPinTypeGPIOOutput(GPIO_PORTF_BASE, GPIO_PIN_0 | GPIO_PIN_1);			//Set PF0 as Output pin
	...
	GPIOPinTypeGPIOInput(GPIO_PORTJ_BASE,GPIO_PIN_0);//Set the PJ0,PJ1 as input pin
